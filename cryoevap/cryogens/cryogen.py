...
class Cryogen:
    """
    Represents a cryogenic fluid and provides methods to calculate its thermodynamic
    and thermophysical properties using CoolProp. This class supports property updates
    for varying conditions and integration over spatial grids, enabling simulation of
    cryogen behavior in engineering applications.

    Attributes:
        R (float): Universal gas constant (J/mol/K).
        name (str): Name of the cryogen (CoolProp fluid string).
        P (float): Pressure in Pa.
        T_sat (float): Saturation temperature in K.
        T_V (float): Vapour temperature in K.
        Tv_avg (float): Average vapour temperature in K.
        rho_L (float): Liquid density in kg/m^3.
        rho_V (float): Vapour density in kg/m^3.
        rho_V_sat (float): Vapour density at the interface in kg/m^3.
        h_L (float): Liquid enthalpy in J/kg.
        h_V (float): Vapour enthalpy in J/kg.
        k_V (float): Vapour thermal conductivity in W/mK.
        k_int (float): Thermal conductivity at the vapour-liquid interface in W/mK.
        cp_V (float): Vapour heat capacity in J/kg/K.
        MW (float): Molar mass in g/mol.
        T_air (float): Reference air temperature in K.
        g (float): Gravitational acceleration in m/s^2.
    
    Methods:
    -------
        __init__(self, name="EmptyCryogen", P=0, T_sat=0, T_V=0, rho_L=0, rho_V=0, h_L=0, h_V=0, k_V=0, cp_V=0, MW=0, T_air=298, g=9.81):
            Initializes a Cryogen instance with the main parameters of the cryogenic fluid.
        rho_ig(self, T=None, P=None):
            Calculates and returns the ideal gas density (kg/m^3) for the given temperature and pressure.
        set_coolprops(self, p=100000):
            Sets the thermodynamic and thermophysical properties of the cryogen using CoolProp for a given pressure.
        update_rho_V(self, z_grid, T_V):
            Updates the vapor density field and its average value by integrating over the spatial grid and the vapor temperature profile.
        update_k_V(self, z_grid, T_V):
            Updates the vapor thermal conductivity and its average value by integrating over the spatial grid and the vapor temperature profile.
        update_cp_V(self, z_grid, T_V):
            Updates the specific heat capacity of vapor and its average value by integrating over the spatial grid and the vapor temperature profile.
    """
    ...
    def set_coolprops(self, p=100000):
        ...
        self.MW       = CP.PropsSI(fluid,'molemass')       # Molar mass / g/mol
